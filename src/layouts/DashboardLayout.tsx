...
const DashboardLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex">
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex flex-col w-64 glass-panel border-r border-slate-900">
        <div className="h-16 flex items-center px-6 border-b border-slate-900/60">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-bold text-[17px] tracking-wide text-white">EMPLOYEE ERP</span>
          </div>
        </div>
      ...
      <aside 
        className={`fixed top-0 bottom-0 left-0 z-50 w-64 bg-slate-900 border-r border-slate-800 flex flex-col transform transition-transform duration-300 lg:hidden ${
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Top Navbar */}
        <header className="h-16 glass-panel border-b border-slate-900/60 flex items-center justify-between px-6 z-30">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              <button
                ...
                className="p-1.5 rounded-lg bg-slate-800/40 hover:bg-slate-800 text-slate-400 hover:text-white border border-slate-800 transition-all relative flex items-center justify-center"
              >
                <Bell size={18} />
                {unreadCount > 0 && (
              ...
              {/* Dropdown Panel */}
              {isNotifOpen && (
                <div className="absolute right-0 mt-2.5 w-80 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-155">
                    ...
                    {notifications.length === 0 ? (
                      ...
                      notifications.map((notif) => (
                        <div
                          ...
                          className={`p-3.5 text-left cursor-pointer transition-all hover:bg-slate-800/40 ${
                            !notif.isRead ? 'bg-indigo-950/5' : ''
                          }`}
                        >
                          <div className="flex justify-between items-start gap-1">
                            <span className={`text-[11px] font-bold ${!notif.isRead ? 'text-indigo-400 font-semibold' : 'text-slate-400'}`}>
                              {notif.title}
                            </span>
                            {!notif.isRead && (
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[13px] text-slate-400 font-medium hidden sm:inline">Active Session</span>
            </div>
          </div>
        </header>
